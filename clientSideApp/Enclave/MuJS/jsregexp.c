...
static void Rp_toString(js_State *J)
{
	...
	strncpy(out, "/", 1);
	strncat(out, re->source, sizeof(re->source));
	strncat(out, "/", 1);
	if (re->flags & JS_REGEXP_G) strncat(out, "g",1);
	if (re->flags & JS_REGEXP_I) strncat(out, "i",1);
	if (re->flags & JS_REGEXP_M) strncat(out, "m",1);
