...
static void dumpnode(Renode *node)
{
	...
	if (!node) { //printf("Empty"); 
		return; }
	switch (node->type) {
	case P_CAT: //printf("Cat("); dumpnode(node->x); printf(", "); dumpnode(node->y); printf(")"); 
		break;
	case P_ALT: //printf("Alt("); dumpnode(node->x); printf(", "); dumpnode(node->y); printf(")"); 
		break;
	case P_REP:
		...
		//printf(")");
		break;
	case P_BOL: //printf("Bol"); 
		break;
	case P_EOL: //printf("Eol"); 
		break;
	case P_WORD: //printf("Word"); 
		break;
	case P_NWORD: //printf("NotWord"); 
		break;
	case P_PAR: //printf("Par(%d,", node->n); 
		dumpnode(node->x); //printf(")"); 
		break;
	case P_PLA: //printf("PLA("); 
		dumpnode(node->x); //printf(")"); 
		break;
	case P_NLA: //printf("NLA("); 
		dumpnode(node->x); //printf(")"); 
		break;
	case P_ANY: //printf("Any"); 
		break;
	case P_CHAR: //printf("Char(%c)", node->c); 
		break;
	case P_CCLASS:
		//printf("Class(");
	...
	case P_NCCLASS:
		...
		//printf(")");
		break;
	case P_REF: //printf("Ref(%d)", node->n); 
		break;
	}
}
...
static void dumpprog(Reprog *prog)
{
	...
	for (i = 0, inst = prog->start; inst < prog->end; ++i, ++inst) {
		...
		case I_JUMP: //printf("jump %d\n", (int)(inst->x - prog->start)); 
			break;
		case I_SPLIT: //printf("split %d %d\n", (int)(inst->x - prog->start), (int)(inst->y - prog->start)); 
			break;
